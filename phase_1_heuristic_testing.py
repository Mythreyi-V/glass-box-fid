...
if xai_method == "SHAP":
    ...
    print("SHAP Explainer Type:", type(explainer))
...
percents = np.arange(0.05, 0.55, 0.05)
...
for percentile in tqdm(percents):
    compiled_precision = []
    compiled_recall = []
    ...
    for i in range(10):
        ...
        tk.append(len(true_features))
        ek.append(len(explanation_features))

        compiled_precision.append(precision)
        compiled_recall.append(recall)
    
    avg_precision.append(np.median(compiled_precision))
    avg_recall.append(np.median(compiled_recall))
    
    true_k.append(np.median(tk))
    exp_k.append(np.median(ek))
    
avg_f1 = []
for i in range(len(avg_precision)):
    recall = avg_recall[i]
    precision = avg_precision[i]
...
f1 = np.array(avg_f1)
rel_idx = np.argmax(f1)
idx = np.arange(0, len(f1))

if min(f1)==max(f1):
    rel_percentile = 'all'
else:
    percents = np.array(percents)[idx]
    rel_percentile = percents[rel_idx]
    
prec = np.array(avg_precision)[idx]
rec = np.array(avg_recall)[idx]

with open(final_folder+'heuristics.json', 'r') as f:
    details = json.load(f)
f.close()

details['Data'] = dataset
details["Model"] = cls_method

details[xai_method+'_Percentile'] = rel_percentile
details["True K"] = np.array(true_k)[idx][rel_idx]
details[xai_method+"_K"] = np.array(exp_k)[idx][rel_idx]
details[xai_method+"_F1"] = f1[rel_idx]
details[xai_method+"_Precision"] = prec[rel_idx]
details[xai_method+"_Recall"] = rec[rel_idx]

with open(final_folder+'heuristics.json', 'w') as f:
    json.dump(details,f)
f.close()
